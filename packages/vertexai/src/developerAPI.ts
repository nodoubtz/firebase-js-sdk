...
import {
  ...
  DeveloperAPIGenerateContentCandidate,
...
/**
 * Maps a Vertex AI {@link GenerateContentRequest} to a format that can be sent to the Developer API.
 *
 * @param generateContentRequest The {@link GenerateContentRequest} to map.
 * @returns A {@link GenerateContentResponse} that conforms to the Developer API format.
 *
 * @throws If the request contains properties that are unsupported by the Developer API.
 */
...
/**
 * Maps a {@link GenerateContentResponse} from the Developer API to the format of the
 * {@link GenerateContentResponse} that we get from VertexAI that is exposed in the public API.
 *
 * @param developerAPIResponse The {@link GenerateContentResponse} from the Developer API.
...
export function mapGenerateContentResponse(
  developerAPIResponse: DeveloperAPIGenerateContentResponse
): GenerateContentResponse {
  const generateContentResponse = {
    candidates: developerAPIResponse.candidates
      ? mapGenerateContentCandidates(developerAPIResponse.candidates)
      : undefined,
    prompt: developerAPIResponse.promptFeedback
      ? mapPromptFeedback(developerAPIResponse.promptFeedback)
      : undefined,
...
/**
 * Maps a Vertex AI {@link CountTokensRequest} to a format that can be sent to the Developer API.
 *
 * @param countTokensRequest The {@link CountTokensRequest} to map.
 * @returns A {@link CountTokensRequest} that conforms to the Developer API format.
 */
...
export function mapGenerateContentCandidates(
  candidates: DeveloperAPIGenerateContentCandidate[]
): GenerateContentCandidate[] {
  ...
  if (mappedCandidates) {
    candidates.forEach(candidate => {
      ...
      if (candidate.safetyRatings) {
        logger.warn(
          "Candidate safety rating properties 'severity', 'severityScore', and 'probabilityScore' are not included in responses from the Developer API. Properties have been assigned to default values."
        );
        candidate.safetyRatings.forEach(safetyRating => {
          safetyRating.severity = HarmSeverity.HARM_SEVERITY_UNSPECIFIED;
...
export function mapPromptFeedback(
  ...
  promptFeedback.safetyRatings.forEach(safetyRating => {
    mappedSafetyRatings.push({
      ...
      severity: HarmSeverity.HARM_SEVERITY_UNSPECIFIED,
      probabilityScore: 0,
      severityScore: 0,
  ...
  logger.warn(
    "PromptFeedback safety ratings' properties severity, severityScore, and probabilityScore are not included in responses from the Developer API. Properties have been assigned to default values."
  );
